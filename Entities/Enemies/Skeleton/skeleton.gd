...
#Nodes
@onready var damage_zone = $Components/SkeletonDealDamage/CollisionShape2D
...
func _process(delta):
	...
	if !state_machine.current_state.name == "Resurection":
		if dir.x < 0:
			sprite.flip_h = true
		else:
			sprite.flip_h = false
...
func take_damage(damage):
	...
	if health_component.is_dead() or dead:
		state_machine.change_state("Death")

func update_damage_zone_position():
	if dir.x == -1:
		damage_zone.position.x = -20
		dect_damage_zone.position.x = -10
	else:
		damage_zone.position.x = 20
		dect_damage_zone.position.x = 10
...
func _on_player_lost(player):
	if player == self.player:
		self.player = null
		is_chase = false
		if !state_machine.current_state.name == "Death" and !state_machine.current_state.name == "Resurection":
